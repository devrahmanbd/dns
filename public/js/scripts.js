...
(function ($) {
  ...
  $(function () {
    var servers = JSON.parse($("#servers").text());
    ...
    if ($("#map").length > 0) {
      $("#map").vectorMap({
        ...
        onMarkerTipShow: function onMarkerTipShow(e, el, index) {
        ...
        onRegionTipShow: function onRegionTipShow(e) {
          e.preventDefault();
        }
      });
    ...
    function getResults(key, id, type, domain, expected) {
      $.ajax({
        url: "fetch/".concat(domain, "/").concat(type, "/").concat(id),
        type: "POST",
        data: {
        ...
        error: function error(data) {
          console.error("Error on: ".concat(servers[key].name, " with message ").concat(data));
          $("#server-" + id + " .status img").attr("src", errorImg);
    ...
    function setHash(domain, type) {
      location.hash = "#/".concat(type, "/").concat(domain);
    }
...
})(jQuery); //Reload if 100 minutes are passed
...
document.querySelectorAll("#blacklist form").forEach(function (el) {
...
function checkBlacklist() {
  ...
  var input = document.querySelector("#blacklist input[name=input]").value;
  ...
  if (trim(input)) {
    ...
    axios.post("/domain/ip/".concat(input)).then(function (response) {
      if (response.data) {
        ...
        if (ip) {
          ...
          blacklists.forEach(function (blacklist) {
            axios.post("/blacklist", {
              ...
              current++;
              document.querySelector("#blacklist form input[type=submit]").value = "Checking (".concat(current, "/").concat(blacklists.length, ")");
              var listed = response.data ? true : false;
              var icon = "fa-check-circle text-success";
              var label_css = "bg-success";
              var label = "Not Listed";
              ...
              document.querySelector("#blacklist-results .row").insertAdjacentHTML("beforeend", "<div class=\"col-12 col-lg-6 col-xl-4 mt-2\">\n                        <div class=\"border rounded p-2\">\n                            <div class=\"row\">\n                                <div class=\"col-8 d-flex align-items-center gap-5\">\n                                    <i class=\"fas ".concat(icon, " ml-1\"></i>\n                                    <div>").concat(blacklist, "</div>\n                                </div>\n                                <div class=\"col-4 text-right\">\n                                    <small class=\"").concat(label_css, " text-white px-2 py-1 rounded\">").concat(label, "</small>\n                                </div>\n                            </div>\n                        </div>\n                    </div>"));
            })["finally"](function () {
...
document.querySelectorAll("#dmarc form").forEach(function (el) {
  el.addEventListener("submit", function (e) {
    e.preventDefault();
...
function checkDmarc() {
  ...
  var input = document.querySelector("#dmarc input[name=input]").value;
  ...
  if (trim(input)) {
    ...
    axios.post("/dmarc/".concat(input)).then(function (response) {
      document.querySelector("#dmarc form input[type=submit]").disabled = false;

      if (response.data) {
        var records = response.data.split(";");

        if (records.length > 0) {
          var tbody = "";

          for (var i = 0; i < records.length; i++) {
            var _records$i$split = records[i].split("=");

            var _records$i$split2 = _slicedToArray(_records$i$split, 2);

            type = _records$i$split2[0];
            value = _records$i$split2[1];
            tbody += "\n                <tr>\n                  <th>".concat(type, "</th>\n                  <td>").concat(value, "</td>\n                  <td><img src=\"/").concat(checkValidity(type, value) ? successImg : errorImg, "\" alt=\"validity-check\"></td>\n                  <td>").concat(getDescription(type), "</td>\n                </tr>\n              ");
          }
...
if (location.hash) {
  var _value = location.hash.replaceAll("#", "");

  if (location.pathname.includes("blacklist")) {
    document.querySelector("#blacklist input[name=input]").value = _value;
    checkBlacklist();
  } else if (location.pathname.includes("dmarc")) {
    document.querySelector("#dmarc input[name=input]").value = _value;
    checkDmarc();
  }
}
})();
